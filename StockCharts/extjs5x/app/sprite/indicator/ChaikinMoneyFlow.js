...
Ext.define('Chartsly.sprite.indicator.ChaikinMoneyFlow', {
     ...
     * @return
     */
    drawSellingPressureArea: function (ctx, list, osLevel) {
        var attr = this.attr,
    ...
    renderAggregates: function (aggregates, start, end, surface, ctx, clip, rect) {
        ...
        var bpLevel = Math.round( 0 * yy + dy) - pixelAdjust;
        var spLevel = Math.round( 0 * yy + dy) - pixelAdjust;
        ...
        if (list.length) {
            ...
            me.drawYLine(ctx, xLen, midLevel);
				
				// Draw and fill the buying pressure area
				me.drawBuyingPressureArea(ctx, list, bpLevel);

				// Draw and fill the selling pressure area
            me.drawSellingPressureArea(ctx, list, spLevel);
